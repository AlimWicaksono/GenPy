...
def display(candidate, start_time):
    ...
    print("{0}\t{1}\t{2}".format(''.join(candidate.Genes), candidate.Fitness, str(time_diff)))
...
class GuessPasswordTests(unittest.TestCase):
    ...
    def guess_password(self, target):
        ...
        best = genetic.get_best(fn_get_fitness, len(target),
    ...
    def test_Random(self):
        length = 150
        target = ''.join(random.choice(self.gene_set) for _ in
                         range(length))
